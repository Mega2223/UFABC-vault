---
tags:
  - computação
  - comunicações
  - cálculo
  - incompleto
authors: Júlio César
---
## Definição

Envolve fazer uma conversão suficientemente fiel de um [[sinal analógico]] para um [[sinal digital]] ou vice-versa.

## Sinal Analógico em Sinal Digital

A conversão de um sinal analógico em um sinal digital envolve discretizar o domínio e contradomínio do sinal, a perda de informação é inevitável uma vez que o domínio analógico não é contável.
### Amostragem
A amostragem é o processo de converter o domínio contínuo do sinal analógico em um domínio discreto por meio de sensorear a amplitude do sinal em um número discreto de instantes. Normalmente esses instantes são distribuídos uniformemente.
### Quantização
Envolve discretizar a amplitude do sinal analógico, isso é, converter o sinal contínuo em um contradomínio discreto de estados.
### Codificação
Para [[#Sinal Binário|sinais binários]] ainda se torna necessária a codificação do sinal, isso envolve converter os estados finitos em combinações de sinais por meio de algum sistema de símbolos.
### DAC
Um [[Conversor Analógico-Digital]] é um circuito capaz de fazer a conversão de um sinal analógico (em forma de uma corrente elétrica) em um sinal digital.
## Sinal Digital em Sinal Analógico



Um [[Conversor Digital-Analógico]] é capaz de converter um conjunto de sinais discretos em um sinal elétrico analógico.